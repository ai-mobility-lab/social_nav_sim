...
import torch
import numpy as np
from policy.pytorchBaselines.a2c_ppo_acktr.utils import init
...
class RNNBase(nn.Module):
    ...
    def __init__(self, config, edge):
        ...
        if edge:
            self.gru = nn.GRU(config["human_human_edge_embedding_size"], config["human_human_edge_rnn_size"])
        # if this is a node RNN
        else:
            self.gru = nn.GRU(config["human_node_embedding_size"]*2, config["human_node_rnn_size"])
...
class HumanNodeRNN(RNNBase):
    ...
    def __init__(self, config):
        ...
        self.rnn_size = config["human_node_rnn_size"]
        self.output_size = config["human_node_output_size"]
        self.embedding_size = config["human_node_embedding_size"]
        self.input_size = config["human_node_input_size"]
        self.edge_rnn_size = config["human_human_edge_rnn_size"]
...
class HumanHumanEdgeRNN(RNNBase):
    ...
    def __init__(self, config):
        ...
        # Store required sizes
        self.rnn_size = config["human_human_edge_rnn_size"]
        self.embedding_size = config["human_human_edge_embedding_size"]
        self.input_size = config["human_human_edge_input_size"]
...
class EdgeAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Store required sizes
        self.human_human_edge_rnn_size = config["human_human_edge_rnn_size"]
        self.human_node_rnn_size = config["human_node_rnn_size"]
...
class SRNN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.human_num = config["num_obstacle"]
        ...
        self.human_node_rnn_size = config["human_node_rnn_size"]
        self.human_human_edge_rnn_size = config["human_human_edge_rnn_size"]
        self.robot_state_size = config["robot_state_size"]
        self.output_size = config["human_node_output_size"]
        ...
        self.robot_linear = init_(nn.Linear(self.robot_state_size, 3))
    ...
    def forward(self, inputs, rnn_hxs, masks):
        ...
        all_hidden_states_edge_RNNs = Variable(
            torch.zeros(1, nenv, self.num_edges, rnn_hxs['human_human_edge_rnn'].size()[-1]).to(self.device))
